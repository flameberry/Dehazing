...
if __name__ == "__main__":
    ...
    trainingDataset = DehazingDataset(
    ...
    # TODO: Abstract this in DehazingModel.py
    trainingDataLoader = tu_data.DataLoader(
        trainingDataset, batch_size=32, shuffle=True, num_workers=3
    )
    validationDataLoader = tu_data.DataLoader(
        validationDataset, batch_size=32, shuffle=True, num_workers=3
    )
        ...
        for step, (haze_image, ori_image) in enumerate(validationDataLoader):
            ...
            torchvision.utils.save_image(
                torchvision.utils.make_grid(
                    torch.cat((haze_image, dehaze_image, ori_image), 0),
                    nrow=ori_image.shape[0],
                ),
                os.path.join(
                    GetProjectDir() / "output", "{}_{}.jpg".format(epoch + 1, step)
                ),
            )
